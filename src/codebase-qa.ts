...
export async function answerQuestion(root: string, query: QAQuery): Promise<QAResult> {
  ...
  const confidence: QAResult["confidence"] =
    topSources.length > 3 ? "high" : topSources.length > 0 ? "medium" : "low";
...
export async function generateOnboardingDocs(root: string): Promise<{
  ...
  if (existsSync(join(root, "requirements.txt")))
    contributingContent += "3. Run `pip install -r requirements.txt`\n";
  ...
  if (existsSync(join(root, "biome.json")))
    contributingContent += "- This project uses Biome for formatting and linting\n";
  if (existsSync(join(root, ".prettierrc")))
    contributingContent += "- This project uses Prettier for formatting\n";
  if (existsSync(join(root, "tsconfig.json")))
    contributingContent += "- TypeScript strict mode is enabled\n";
  ...
  try {
    if (await graphExists(root)) {
      ...
      const entryPoints = graph.nodes.filter(
        (n) =>
          n.type === "function" &&
          (n.name === "main" || n.name === "handler" || n.name === "start"),
      );
    ...
    archContent +=
      "> CodeGraph data not available. Run `scan_codebase` to generate architecture overview.\n\n";
  }
