 ...
 */
export function logFailure(record: Omit<FailureRecord, "id" | "timestamp" | "resolved">): FailureRecord {
...
export function analyzeFailures(): {
  ...
  for (const [errorPrefix, group] of groups) {
    ...
    } else if (errorPrefix.toLowerCase().includes("not found") || errorPrefix.toLowerCase().includes("enoent")) {
      fix = "Ensure path exists before access. Use existsSync() check or try/catch.";
    } else if (errorPrefix.toLowerCase().includes("rate limit") || errorPrefix.toLowerCase().includes("429")) {
      fix = "Add exponential backoff between retries. Wait at least 60s before retry.";
    } else if (errorPrefix.toLowerCase().includes("permission") || errorPrefix.toLowerCase().includes("denied")) {
      fix = "Add required permission to settings or use an alternative tool.";
    } else if (errorPrefix.toLowerCase().includes("parse") || errorPrefix.toLowerCase().includes("syntax")) {
    ...
    writeFileSync(
      ...
      [
        ...
        loadCorrections()
          ...
          .map((c) => `- **${c.pattern}**: ${c.fix} (applied ${c.appliedCount}x)`).join("\n"),
        "",
        "---",
