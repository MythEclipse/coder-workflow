...
test("kotlinParser extracts standard and data classes", () => {
  const source = `
    class StandardClass {
      fun doWork() {}
    }
  ...
  const symbols = kotlinParser.extractSymbols(sanitized, "src/Model.kt");
...
test("kotlinParser extracts objects and companion objects", () => {
  const source = `
  ...
  `;
  const sanitized = kotlinParser.sanitize(source);
  const symbols = kotlinParser.extractSymbols(sanitized, "src/SingletonManager.kt");
  const names = symbols.map((s) => s.name);
...
test("kotlinParser extracts extension functions", () => {
  const source = `
  ...
  `;
  const sanitized = kotlinParser.sanitize(source);
  const symbols = kotlinParser.extractSymbols(sanitized, "src/Extensions.kt");
  const names = symbols.map((s) => s.name);
  assert.ok(names.includes("removeSpaces"));
});

test("kotlinParser parses imports with aliases", () => {
  const source = `
    import com.example.Foo
    import com.example.Bar as Baz
  `;
  const imports = kotlinParser.extractImports(source);
  assert.ok(imports.includes("com.example.Foo"));
  assert.ok(imports.includes("com.example.Bar"));
});

test("kotlinParser resolves Java/Kotlin package imports correctly", () => {
  ...
  const resolved = kotlinParser.resolveImportTarget(
  ...
  const resolvedJava = kotlinParser.resolveImportTarget(
