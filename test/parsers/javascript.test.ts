...
test("javascriptParser extracts functions, classes, and const arrow functions", () => {
  const source = `
  ...
  const symbols = javascriptParser.extractSymbols(sanitized, "src/index.js");
...
test("typescriptParser extracts interfaces and abstract classes", () => {
  const source = `
    export interface UserData {}
    abstract class BaseHandler {}
  `;
  const sanitized = typescriptParser.sanitize(source);
  const symbols = typescriptParser.extractSymbols(sanitized, "src/types.ts");
  const names = symbols.map((s) => s.name);
  assert.ok(names.includes("UserData"));
  assert.ok(names.includes("BaseHandler"));
});

test("javascriptParser parses default and destructured imports", () => {
  const source = `
  ...
  `;
  const importMap = javascriptParser.parseImports(source);
...
test("javascriptParser extracts component usage", () => {
  const source = `
  ...
  const edges = typescriptParser.extractRelationshipEdges(sanitized, symbols, symbolByName);
  const targetNames = edges.map((e) => e.evidence);
  assert.ok(targetNames.includes("BaseUser"));
});

test("javascriptParser resolves relative imports", () => {
  const filePaths = new Set(["src/utils/helper.ts", "src/utils/index.ts"]);

  const resolved = typescriptParser.resolveImportTarget("./helper", "src/utils/main.ts", filePaths);
  assert.equal(resolved, "src/utils/helper.ts");

  const resolvedDir = typescriptParser.resolveImportTarget(".", "src/utils/main.ts", filePaths);
